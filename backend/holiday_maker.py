...
def find_nearest_hotels(coordinates: tuple[str], holiday_details: json) -> list:
    ...
    api_host_url = "priceline-com-provider.p.rapidapi.com"

    coordinates_url = f"https://{api_host_url}/v1/hotels/locations-by-geo"
    headers = {
    ...
    city_response = requests.get(
        coordinates_url, headers=headers, params=coordinates_querystring, timeout=30)
    ...
    hotel_response = requests.get(
        hotel_search_url, headers=headers, params=hotel_querystring, timeout=30)
...
def generate_holiday(holiday_details: json) -> str:
    ...
    system_content_spec = """
        ...
        """
    user_content_spec = f"""I am planning a holiday for {adults_number} adults and
        {children_number} children,going to {user_destination} for {holiday_duration}.
        What is there to do?"""
